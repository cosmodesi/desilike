...
Supported mock types (match full_shape.py):
  ...
  - holi_cutsky / cutsky : HOLI cutsky P(k) mean/covariance + window matrix
...
# Ensure this checkout is imported before any environment-installed desilike.
...
"""
for _desi_clustering_path in [
    "/global/homes/j/jiaxi/codes/desi-clustering",
    "/global/common/software/desi/users/adematti/perlmutter/cosmodesiconda/20260321-1.0.0/code/desi-clustering/main/lib/python3.12/site-packages",
]:
    if os.path.isdir(_desi_clustering_path) and _desi_clustering_path not in sys.path:
        sys.path.insert(0, _desi_clustering_path)
"""
try:
    from clustering_statistics import tools, box_tools  # noqa: F401
    import lsstypes as types
except Exception as exc:
    raise ImportError(
        "run_desilike_mockchallenge.py now requires desi-clustering loaders "
        "(clustering_statistics, lsstypes) to import successfully."
    ) from exc
"""Loaded automatically by Python at startup when this directory is on sys.path.
"""
...
HOLI_TRACER_INFO = {
    "LRG1": {"zeff": 0.510, "tracer_name": "LRG", "zrange": (0.4, 0.6)},
    "LRG2": {"zeff": 0.706, "tracer_name": "LRG", "zrange": (0.6, 0.8)},
    "LRG3": {"zeff": 0.922, "tracer_name": "LRG", "zrange": (0.8, 1.1)},
    "ELG1": {"zeff": 0.955, "tracer_name": "ELG_LOPnotqso", "zrange": (0.8, 1.1)},
    "ELG2": {"zeff": 1.321, "tracer_name": "ELG_LOPnotqso", "zrange": (1.1, 1.6)},
    "QSO": {"zeff": 1.484, "tracer_name": "QSO", "zrange": (0.8, 2.1)},
}

HOLI_CUTSKY_ZEFF = {tracer: info["zeff"] for tracer, info in HOLI_TRACER_INFO.items()}

HOLI_BASE_DIR = Path(
    "/global/cfs/cdirs/desi/science/cai/desi-clustering/dr2/summary_statistics/full_shape/base/holi-v3-altmtl"
)
HOLI_WINDOW_TEMPLATE = str(HOLI_BASE_DIR / "mock0" / "window_mesh2_spectrum_poles_*_GCcomb_weight-default-FKP.h5")
HOLI_NMOCKS = 1000
...
def allowed_tracers_for(mock_type: str) -> set[str]:
    ...
    if mt == "holi_cutsky":
        return set(HOLI_CUTSKY_ZEFF.keys())
    if mt == "cubic":
...
def z_eff_for(tracer: str, mock_type: str) -> float:
    ...
    if mt == "holi_cutsky":
        return float(HOLI_CUTSKY_ZEFF[tr])
    if mt == "cubic":
...
# -----------------------------------------------------------------------------
# HOLI cutsky helpers (mean/cov from mocks + window matrix)
# -----------------------------------------------------------------------------
def _holi_catalog_dict(tracer: str, imock=None, weight: str = "default-FKP", version: str = "holi-v3-altmtl"):
    tr = str(tracer).upper()
    info = HOLI_TRACER_INFO[tr]
    tracer_name = str(info["tracer_name"])
    zrange = tuple(info["zrange"])
    catalog = dict(version=version, tracer=tracer_name, zrange=zrange, region="GCcomb", weight=weight)
    if imock is not None:
        catalog["imock"] = int(imock)
    return catalog


def _holi_get_stats_fn(tracer: str, *, holi_base_dir: Path, kind: str, imock=None, cut: bool = True, auw: bool = False) -> str:
    catalog = _holi_catalog_dict(tracer, imock=imock)
    return tools.get_stats_fn(stats_dir=holi_base_dir, kind=kind, catalog=catalog, cut=cut, auw=auw)


def _holi_mock_pk_path(tracer: str, imock: int, holi_base_dir: Path = HOLI_BASE_DIR) -> Path:
    tr = str(tracer).upper()
    info = HOLI_TRACER_INFO[tr]
    zmin, zmax = info["zrange"]
    return holi_base_dir / f"mock{int(imock)}" / (
        f"mesh2_spectrum_poles_{info['tracer_name']}_z{zmin:.1f}-{zmax:.1f}_GCcomb_weight-default-FKP.h5"
    )

def _holi_window_path(tracer: str, window_template: str = HOLI_WINDOW_TEMPLATE) -> Path:
    tr = str(tracer).upper()
    info = HOLI_TRACER_INFO[tr]
    zmin, zmax = info["zrange"]
    token = f"{info['tracer_name']}_z{zmin:.1f}-{zmax:.1f}"
    return Path(window_template.replace("*", token))


def load_holi_vector_and_covariance(
    tracer: str,
    *,
    kmin_cut: float = 0.02,
    kmax_cut: float = 0.30,
    ...
    found_paths = []
    missing_imocks = []
    for imock in range(HOLI_NMOCKS):
        fn = _holi_mock_pk_path(tracer, imock, holi_base_dir=HOLI_BASE_DIR)
        if fn.exists():
            found_paths.append(fn)
            stats.append(types.read(fn))
        else:
            missing_imocks.append(imock)
    if not stats:
        raise FileNotFoundError(f"[holi] No P(k) files found for tracer={tracer} in {HOLI_BASE_DIR}")
    """
    if missing_imocks:
        raise FileNotFoundError(
            f"[holi] Missing {len(missing_imocks)} P(k) files for tracer={tracer}; "
            f"first missing mock indices: {missing_imocks[:10]}"
        )
    """
    mean_stat = types.mean(stats)
    mean_stat = mean_stat.get(ells=list(present_ells)).select(k=(float(kmin_cut), float(kmax_cut)))
    k_out = np.asarray(mean_stat.get(ells=0).k).reshape(-1)
    data_full = np.asarray(mean_stat.value()).reshape(-1)

    cov_all = np.asarray(types.cov(stats).at.observable.match(mean_stat).value())
    if cov_scale != 1.0:
        cov_all = float(cov_scale) * cov_all

    shotnoise = float(np.asarray(stats[0].get(ells=0).values("shotnoise")).reshape(-1)[0])
    ncut = len(k_out)
    data_vec, cov_mat = select_data_and_cov(
        data_full,
        cov_all,
        present_ells=present_ells,
        requested_ells=requested_ells,
        start=0,
        Ncut=ncut,
    )
    nmocks = len(found_paths)
    ndata = int(data_vec.size)
    hartlap = (nmocks - 1.0) / (nmocks - ndata - 2.0)
    if hartlap <= 0:
        raise ValueError(
            f"[holi] Invalid Hartlap factor for tracer={tracer}: nmocks={nmocks}, ndata={ndata}, hartlap={hartlap}"
        )
    cov_mat = cov_mat * hartlap
    return data_vec, cov_mat, k_out, shotnoise


def load_holi_window_matrix(
    tracer: str,
    *,
    kmin_cut: float = 0.02,
    kmax_cut: float = 0.30,
    requested_ells: tuple[int, ...] = (0, 2),
    theory_ells: tuple[int, ...] = (0, 2, 4),
) -> tuple[np.ndarray, np.ndarray, np.ndarray, Path]:
    window_path = _holi_window_path(tracer)
    if not window_path.exists():
        raise FileNotFoundError(f"[holi] Missing window file for tracer={tracer}: {window_path}")
    observable = types.read(_holi_mock_pk_path(tracer, 0, holi_base_dir=HOLI_BASE_DIR))
    observable = observable.get(ells=list(requested_ells)).select(k=(float(kmin_cut), float(kmax_cut)))
    window_correlation = types.read(window_path).at.observable.match(observable)
    k_out = np.asarray(observable.get(ells=0).k).reshape(-1)
    wmat = np.asarray(window_correlation.value())
    n_theory_ells = len(tuple(theory_ells))
    if wmat.ndim != 2:
        raise ValueError(f"[holi] Expected 2D window matrix for tracer={tracer}, got shape={wmat.shape}")
    if wmat.shape[1] % n_theory_ells != 0:
        raise ValueError(
            f"[holi] Window matrix shape {wmat.shape} is incompatible with theory_ells={tuple(theory_ells)} "
            f"for tracer={tracer}"
        )
    nkin = wmat.shape[1] // n_theory_ells
    if nkin == k_out.size:
        kin_all = k_out.copy()
    else:
        kin_all = None
        theory_obj = getattr(window_correlation, "theory", None)
        if theory_obj is not None:
            if hasattr(theory_obj, "k"):
                try:
                    kin_all = np.asarray(theory_obj.k).reshape(-1)
                except Exception:
                    kin_all = None
            if kin_all is None and hasattr(theory_obj, "get"):
                for ell in theory_ells:
                    for label in (ell, str(ell)):
                        try:
                            kin_all = np.asarray(theory_obj.get(ells=label).k).reshape(-1)
                            break
                        except Exception:
                            continue
                    if kin_all is not None:
                        break
        if kin_all is None:
            raise ValueError(
                f"[holi] Could not infer theory k-grid from window for tracer={tracer}; "
                f"window shape={wmat.shape}, k_out size={k_out.size}"
            )
    return k_out, kin_all, wmat, window_path


def build_holi_observables_for_tracer(
    tracer: str,
    ps_theory,
    *,
    ells: tuple[int, ...],
    ...
    k_out: np.ndarray | None = None,
    kin: np.ndarray | None = None,
    wmatrix: np.ndarray | None = None,
    window_path: Path | None = None,
):
    if tuple(ells) != (0, 2):
        raise ValueError(f"[holi] Supported --ells are only '0,2'. Got {ells!r}.")

    shotnoise = None
    if data_vec is None or cov_mat is None or k_out is None:
        data_vec, cov_mat, k_out, shotnoise = load_holi_vector_and_covariance(
            tracer,
            kmin_cut=kmin_cut,
            kmax_cut=kmax_cut,
            requested_ells=ells,
    ...
    if kin is None or wmatrix is None or window_path is None:
        k_out_w, kin, wmatrix, window_path = load_holi_window_matrix(
            tracer,
            kmin_cut=kmin_cut,
            kmax_cut=kmax_cut,
            requested_ells=ells,
        ...
        if not np.allclose(k_out, k_out_w, rtol=0.0, atol=1e-10):
            raise ValueError(f"[holi] data k-grid and window observable k-grid mismatch for tracer={tracer}.")
    ...
    obs = TracerPowerSpectrumMultipolesObservable(
        ...
        k=k_out,
        kin=kin,
        ellsin=list(theory_ells),
        wmatrix=wmatrix,
    )
    return [obs], cov_mat, kin, shotnoise, window_path
...
def parse_args():
    ...
    p.add_argument(
        "--tracers",
        nargs="+",
        default=["LRG1", "LRG2", "LRG3", "ELG1", "ELG2", "QSO"],
        help="List of tracers. For holi_cutsky: LRG1, LRG2, LRG3, ELG1, ELG2, QSO. "
             "For cubic: ABACUS_MC2_LRG, MC4_LRG_0P7, ... "
    ...
    p.add_argument(
        "--mock-type",
        default="holi_cutsky",
        choices=["holi_cutsky", "cubic", "cutsky", "synthetic", "weiliu", "cubic+weiliu"],
        help="holi_cutsky: HOLI cutsky data+cov+window; "
             "cubic: Mike cubic-box data+EZ cov; cutsky: legacy alias/table-1 naming; "
...
def emu_filename(
    ...
    *,
    ...
    nk = (
        f"_nkin{int(nkin)}"
        if (nkin is not None and str(mock_type).lower() in ("cubic", "holi_cutsky", "cutsky"))
        else ""
    )
...
def main():
    ...
    if mt == "cutsky" and all(t.strip().upper() in HOLI_CUTSKY_ZEFF for t in args.tracers):
        mt = "holi_cutsky"
    elif mt == "cutsky":
        raise NotImplementedError(
            "Legacy table-1 cutsky is still not wired here. Use --mock-type holi_cutsky for the HOLI cutsky mock challenge."
        )
    ...
    if mt == "holi_cutsky" and args.add_bispectrum:
        raise ValueError("mock_type='holi_cutsky' currently supports P(k) only (no bispectrum).")
    ...
    for tracer in tracers:
        ...
        if mt_tr == "synthetic":
        ...
        elif mt_tr == "holi_cutsky":
            if tuple(req_ells) != (0, 2):
                raise ValueError(f"[holi] Only --ells 0,2 is supported right now. Got {tuple(req_ells)!r}.")
            theory_ells = (0, 2, 4)
            data_vec, cov_mat, k_out, shotnoise = load_holi_vector_and_covariance(
                tracer,
                kmin_cut=args.kmin_cut,
            ...
            k_out_w, kin_expected, wmatrix, window_path = load_holi_window_matrix(
                tracer,
                kmin_cut=args.kmin_cut,
                kmax_cut=args.kmax_cut,
                requested_ells=tuple(req_ells),
            ...
            obs_list, cov_mat, kin_expected, _, window_path = build_holi_observables_for_tracer(
                tracer,
                ps_theory,
                ells=tuple(req_ells),
                theory_ells=theory_ells,
            ...
            kin_expected = np.asarray(kin_expected).reshape(-1)
            nkin_for_name = int(kin_expected.size)
            ps_theory.init.update(k=kin_expected, ells=list(theory_ells))

            cov_like = ObservableCovariance(cov_mat, observables=covmeta_from_observables(obs_list))
            if rank == 0:
                print(f"[holi] ({tracer}) using window {window_path}")

        else:
            raise ValueError(f"Unhandled mock_type={mt_tr!r}")

        emu_kr_max = float(args.kmax_cut) if mt_tr in ("synthetic", "weiliu", "holi_cutsky") else float(args.kr_max)
