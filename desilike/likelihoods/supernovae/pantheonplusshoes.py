...
class PantheonPlusSHOESSNLikelihood(BaseSNLikelihood):
    ...
    def initialize(self,*args, **kwargs):
        super(PantheonPlusSHOESSNLikelihood, self).initialize(*args, **kwargs)
    ...
    def calculate(self, Mb=0):
        ...
        self.flattheory= z*np.nan
        
        # Use Cepheids host distances as theory 
        self.flattheory[np.array(self.is_calibrator,dtype='bool')] = self.cepheid_distance[np.array(self.is_calibrator,dtype='bool')] 
        
        # Compute predictions at those redshifts that are not used as calibrators
        zcmb = self.light_curve_params['zcmb'][~np.array(self.is_calibrator,dtype='bool')]
        zhel = self.light_curve_params['zhel'][~np.array(self.is_calibrator,dtype='bool')]
        
        self.flattheory[~np.array(self.is_calibrator,dtype='bool')] = 5 * np.log10(self.cosmo.luminosity_distance(zcmb)/self.cosmo['h']) + 25 + 5 * np.log10((1 + zhel) / (1 + zcmb))
        self.flatdata = self.light_curve_params['mb'] - Mb 
        
        super(PantheonPlusSHOESSNLikelihood, self).calculate()
    ...
    def read_light_curve_params(self, fn):
        ...
        self.filter=(data['zHD']>0.01) | (np.array(data['IS_CALIBRATOR'],dtype=bool)) # Select only those SNe at z>0.01 or the ones used as calibrators
        
        data=data[self.filter]
        
        self.is_calibrator = data['IS_CALIBRATOR']
        self.cepheid_distance = data['CEPH_DIST']
