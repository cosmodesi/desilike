...
from desilike.samplers.blackjax import make_steps_factory
...
class _BlackJAXKernel(Kernel):
    ...
    def init(self, logposterior, params, rng, **context):
        ...
        kernel = kernel_type(logposterior, **self.kernel_args, **self.fixed_kernel_args)
        self._make_steps = make_steps_factory(kernel.step)
        self._kernel = kernel
        self._state = None   # initialised lazily on first run / after adapt
        self._initial_position = context['initial_position']
        self._total_likelihood_evaluations = 0

    def _get_or_init_state(self, initial_position=None):
        if self._state is None:
            position = initial_position if initial_position is not None else self._initial_position
            try:
                self._state = self._kernel.init(position)
            except TypeError:
                rng_key = jax.random.PRNGKey(int(self._rng.integers(2**32)))
                self._state = self._kernel.init(position, rng_key)
        return self._state

    def run(self, n_steps):
        state = self._get_or_init_state()
        rng_key = jax.random.PRNGKey(int(self._rng.integers(2**32)))
        ...
        samples  = np.column_stack([np.asarray(all_states.position[name]) for name in self._params])
