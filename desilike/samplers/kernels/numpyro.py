...
class _NumpyroKernel(Kernel):
    ...
    def init(self, logposterior, params, rng, **context):
        ...
        # Build flat layout for extracting per-parameter arrays from numpyro samples.
        layout = []
        col = 0
        for name in params:
        ...
        # numpyro minimises the potential; potential_fn receives a rescaled-space dict.
        # logposterior already handles the forward transform internally.
        potential_fn = lambda pos_dict: -logposterior(pos_dict)
        self._potential_fn = potential_fn

        self._numpyro_kernel = getattr(numpyro.infer, self._numpyro_cls)(
            potential_fn=potential_fn, **self.kernel_kwargs)

        self._current_position = context['initial_position']   # {name: shaped_array}
    ...
    def run(self, n_steps):
        ...
        samples = np.column_stack([
            np.asarray(samples_dict[name]).reshape(n_steps, size)
            for name, size, shape, col in self._layout
        ])
