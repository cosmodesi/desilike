...
class _Prior:
    """Prior wrapper for ``pocoMC`` built from transformed parameter objects."""
    ...
    def logpdf(self, x):
        logp = np.zeros(len(x))
        for param_idx, dist in enumerate(self.dists):
            logp += np.asarray(dist.logpdf(x[:, param_idx]))
        return logp
    ...
    @property
    def bounds(self):
        return np.array([dist.limits for dist in self.dists]).astype(float)
    ...
    def run(self, loglikelihood, logprior, prior_transform, pool, rng, ndim,
        ...
        if pool.main:
            if self._sampler is None:
                if params is None:
                    raise ValueError(
                        "PocoMC kernel requires 'params' (transformed parameter objects) "
                        "to build the prior.  Pass params=self._transformed_params from "
                        "NestedSampler.run_sampler()."
                    )
                prior = _Prior(params, rng=rng)

                # When there are derived parameters, keep (log_l, derived) tuples so
                # pocomc stores blobs; otherwise return plain scalars.
                if has_derived:
                    _likelihood_fn = loglikelihood
                else:
                    _likelihood_fn = lambda batch: [result[0] for result in loglikelihood(batch)]

                init_kwargs = update_kwargs(
                    dict(**self._kwargs), 'pocoMC',
                    prior=prior, likelihood=_likelihood_fn, n_dim=ndim,
                    pool=pool,
