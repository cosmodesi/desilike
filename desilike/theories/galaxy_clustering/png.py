...
class PNGTracerPowerSpectrumMultipoles(BaseTracerPowerSpectrumMultipoles):
    ...
    def _params(cls, params, tracers=None, mode='b-p'):
        ...
        # tnl_exc_loc (local tauNL SY-EXCESS = tauNL - (6/5 fnl)^2) is fixed to 0 by
        # default (see png.yaml), so fNL-only analyses are unaffected; free it for
        # tauNL / joint fits. Only the 'b-p' / 'bphi' modes expose bphi, which the
        # tauNL term needs; 'bfnl' mode is left as-is (fNL-only).
        if mode == 'bphi':
            keep_params += ['fnl_loc', 'tnl_exc_loc', 'bphi']
        elif mode == 'b-p':
            keep_params += ['fnl_loc', 'tnl_exc_loc', 'p']
        elif mode == 'bfnl':
    ...
    def calculate(self, **params):
        params = self.decode_params(params, defaults=dict(b1=1., sigmas=0., sn0=0., bphi=1., p=1., bfnl_loc=0., tnl_exc_loc=0.))
        ...
        # power -- a steeper k^-4 shape (isotropic -> monopole only), with
        # (alpha^2 vs alpha for fNL). Rank-one in tracer space (bphiX bphiY).
        # tnl_exc_loc is fixed to 0 by default (png.yaml)
        ...
        # power is proportional to the Suyama-Yamaguchi EXCESS, tauNL - (6/5 fnl)^2
        # (no stochasticity when SY is saturated). Since we add (25/36) tnl_exc_loc
        # bphi^2 alpha^2 on TOP of the Kaiser Delta_b^2, this parameter is the
        # SY-excess:  tnl_exc_loc = tauNL - (6/5 fnl_loc)^2 = tauNL - (36/25) fnl^2.
        #   * fnl-only (tnl_exc_loc=0): unchanged, includes the standard Delta_b^2.
        #   * tnl-only (fnl_loc=0): tnl_exc_loc == tauNL exactly (validation case).
        #   * joint: TRUE tauNL = tnl_exc_loc + (36/25) fnl_loc^2 (derived param).
        # NB: tnl_exc_loc >= 0 positivity prior is exactly the SY bound.
        # NB: not 'rNL' -- the (5/6)^2 convention rNL = (25/36)*tauNL - fnl^2 = (25/36)*tnl_exc_loc.
        tnl_exc_loc = params['tnl_exc_loc']
        if bphiX is not None:
            pkmu = pkmu + jac * fog * (25. / 36.) * tnl_exc_loc * bphiX * bphiY * alpha**2 * pk_obs
