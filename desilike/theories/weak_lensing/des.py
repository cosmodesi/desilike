...
class DESWeakLensing3x2pt(BaseCalculator):
    ...
    def initialize(self, *args, cosmo=None, fiducial='DESI', des_model='DES_3YR', ia_model='TATT', Limber=False, Weyl=False, fourier='binned_bessels',
        ...
        if year == '1YR':
            ...
            self.Limber = True  # Limber approximation used in DES Y1
            ...
            self.nzbins = 4
            self.nwbins = 6
            self.Limber = False  # Limber approximation not used in DES Y3
        ...
        nz_source = jnp.array(nz_source)
        self.zmid = nz_source[:, 1]
        self.zbin_sp = [nz_source[:, b + 3] for b in range(self.nzbins)]
        self.zbin_sp = jnp.array(self.zbin_sp)
        nz_lens = np.loadtxt(os.path.join(data_path, 'DES_{}_final_nz_lens.dat'.format(year)))
        nz_lens = jnp.array(nz_lens)
        assert (jnp.array_equal(nz_lens[:, 1], self.zmid))
        self.zbin_w_sp = [nz_lens[:, b + 3] for b in range(self.nwbins)]
        self.zbin_w_sp = jnp.array(self.zbin_w_sp)
        self.zmax = self.zmid[-1]
        self.z = self.zmid[self.zmid <= self.zmax]

        params = self.init.params.select(basename='DES_*')
