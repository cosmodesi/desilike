...
- `Dynesty` (:class:`~desilike.samplers.DynestySampler`): A pure-Python dynamic nested sampling code. The slice sampling mode in this sampler is similar to that of `PolyChord`.
- `nautilus` (:class:`~desilike.samplers.NautilusSampler`): A pure-Python importance nested sampling code. This is an evolution of `MultiNest`'s importance nested sampling (INS) mode and incorporates neural networks to improve sampling efficiency.
- `pocoMC` (:class:`~desilike.samplers.PocoMCSampler`): An implementation of preconditioned Monte Carlo (PMC), which is an extension of Sequential Monte Carlo. This code leverages normalizing flows to enhance sampling efficiency.

If you use any of these classes in your published work, please make sure to cite the corresponding papers linked in the API documentation.
...
- Metropolis-Hastings MCMC (:class:`~desilike.samplers.MetropolisHastingsSampler`): The classical MCMC algorithm. The version employed in `desilike` supports the fast-and-slow decomposition described in `cobaya`.
- Hamiltonian Monte-Carlo (HMC, :class:`~desilike.samplers.HMCSampler`), No-U-Turn Sampler (NUTS, :class:`~desilike.samplers.NoUTurnSampler`), and Microcanonical Langevin Monte Carlo (MCLMC, :class:`~desilike.samplers.MCLMCSampler`): Classes of MCMC algorithms that leverage the derivative of the posterior to improve chain mixing. All three samplers are implemented via the `blackjax` package.
- `emcee` (:class:`~desilike.samplers.EmceeSampler`): The affine-invariant sampler popular in astronomy. It proposes new points by utilizing the positions of other walkers, i.e., chains.
- `zeus` (:class:`~desilike.samplers.ZeusSampler`): An ensemble slice sampling MCMC algorithm. Like `emcee`, this sampler is insensitive to linear correlations.

The implementation of all samplers is derived from the :class:`~desilike.samplers.base.MarkovChainSampler` class and differ only in their initialization and how they advance the chain via the :meth:`~desilike.samplers.base.MarkovChainSampler.run_sampler` method. In particular, all samplers use the same run method, :meth:`~desilike.samplers.base.MarkovChainSampler.run`. The run method defines how long the chain is run. The following convergence criterion can be employed.

* Gelman-Rubin statistic: Maximum value of the `Gelman-Rubin statistic <https://en.wikipedia.org/wiki/Gelman-Rubin_statistic>`_ :math:`R` across all parameters. See :meth:`~desilike.statistics.diagnostics.gelman_rubin`.
* Effective sample size (ESS): Minimum value for the ESS per chain. The ESS is defined as :math:`n / \tau` where :math:`n` is the length of the chain and :math:`\tau` the integrated autocorrelation time, see :meth:`~desilike.statistics.diagnostics.integrated_autocorrelation_time`.
