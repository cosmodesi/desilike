...
from desilike.profilers import MinuitProfiler  # iminuit wrapper

# JAX settings (optional but nice for fkpt)
...
def parse_args():
    ...
    p.add_argument("--mode", choices=["mcmc", "map"], default="mcmc",
                   help="Choose 'mcmc' to run MCMCSampler, or 'map' to run MinuitProfiler (MAP).")
    ...
    p.add_argument("--kc", type=float, default=0.01, help="Scale for scale transition between bins (k_c).")
...
def emu_filename(tag: str,
    ...
    """Per-tracer PT emulator filename with optional binning tags and k_c."""
...
def main():
    ...
    # IO setup
    workdir = Path("/n/home12/cgarciaquintero/DESI/MG_validation/fR_noiseless")
    ...
    tracer_table = [
        ("BGS",  "BGS",  1.5521, 0.295),
        ("LRG1", "LRG",  1.9616, 0.510),
        ("LRG2", "LRG",  1.9616, 0.706),
        ("LRG3", "LRG",  1.9616, 0.919),
        ("ELG",  "ELG",  1.3329, 1.317),
        ("QSO",  "QSO",  2.1752, 1.492),
    ]
    ...
    bbn_prior = None if args.skip_bbn_prior else {'dist': 'norm', 'loc': 0.0219, 'scale': 0.00055}
    ...
    if args.redshift_bins or args.scale_bins:
        prefix += "_binning"
        if args.redshift_bins: prefix += "_z"
        if args.scale_bins:    prefix += f"_k_kc{args.kc:g}"
    else:
        if args.force_GR:
            prefix += "_GR"
        else: 
    ...
    cosmo = None
    for file_tag, tracer_tag, b1_val, z_tracer in tracer_table:
        namespace = file_tag.lower()

        # load data
        k_all   = np.loadtxt(workdir / f"{file_tag}_{fid_model}_AP_k.txt")
        P_all   = np.loadtxt(workdir / f"{file_tag}_{fid_model}_AP_P0P2P4.txt")
        cov_all = np.loadtxt(workdir / f"{file_tag}_{fid_model}_AP_cov.txt")
        ...
        if cosmo is None:
            ...
            for name, scale, delta in [('h', 0.001, 0.03),
                                       ('omega_cdm', 0.001, 0.007),
                                       ('logA', 0.001, 0.05)]:
                if name in cosmo.init.params:
                    q = cosmo.init.params[name]
                    q.update(ref={'dist': 'norm', 'loc': q.value, 'scale': scale}, delta=delta)

            # MG knobs
            if not (args.redshift_bins or args.scale_bins):
                if args.force_GR:
                    cosmo.init.params.data.append(parameter.Parameter(basename='mu0', value=0.0, fixed=True))
                else:
                    cosmo.init.params.data.append(parameter.Parameter(basename='mu0', value=0.0, fixed=True))
                    cosmo.init.params['mu0'].update(fixed=False,
                ...
                for nm in ['mu1','mu2','mu3','mu4']:
                    ...
                    cosmo.init.params[nm].update(fixed=False,
                                                 prior={'dist':'uniform','limits':[-2.0,4.0]},
                                                 ref={'dist':'norm','loc':1.0,'scale':0.05},
                                                 delta=0.3)
                for nm, val in [('z_div', 1.0), ('z_TGR', 2.0), ('z_tw', 0.05),
                                ('k_tw', 0.001), ('k_c', args.kc),
        ...
        theory.init.update(freedom=args.freedom, prior_basis='standard',
                           ...
                           k=k_out, ells=list(ells),
                           model=args.MG_model, mg_variant=args.mg_variant,
                           beyond_eds=beyond_eds, rescale_PS=True
                           )
        ...
        for name in ['b2','bs','b3']:
            if name in theory.params:
                theory.params[name].update(
                    ref={'dist':'norm','loc':0.0,'scale':0.1},
                    prior={'dist':'uniform','limits':(-50.0,50.0)}
                )
        for p in theory.params.select(basename=['alpha0','alpha2','alpha4','sn0','sn2']):
            if p.varied:
                p.update(derived='.marg')
        for p in theory.params.select(basename=['b1','b2','bs','b3','alpha0','alpha2','alpha4','sn0','sn2']):
        ...
        if args.create_emu and rank == 0:
            if emu_path.exists():
                ...
                _ = theory.pt()  # warm up
                emu_engine = TaylorEmulatorEngine(method='finite', order=4)
    ...
    if args.mode == "mcmc":
        ...
        if args.resume:
            ...
            if not existing:
                print(f"[resume] No existing files found for {prefix}; starting fresh 4 chains.")
                chains_arg = 4
            else:
                print(f"[resume] Resuming {len(existing)} chains:")
                for f in existing: print("  -", f)
        ...
        sampler.run(check={'max_eigen_gr': 0.1}, check_every=2500, max_iterations=50000)
...
#            mpicomm=MPI.COMM_WORLD,
        ...
        raise ValueError(f"Unknown --mode {args.mode!r}; expected 'mcmc' or 'map'")
