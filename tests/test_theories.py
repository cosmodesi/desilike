...
def test_galaxy_clustering_imports():
    ...
    # emulator-backed fkpt variants
    assert hasattr(mod, "MgEmulatorCosmology")
    assert hasattr(mod, "fkpt_pkemu_TracerPowerSpectrumMultipoles")
    assert hasattr(mod, "fkpt_pkemu_TracerBispectrumMultipoles")
...
def test_fkpt_pkemu_runs_in_gr_limit():
    """
    End-to-end smoke test for the emulator-backed fkpt power spectrum.

    Skipped unless the trained ISiTGR MG emulators are on disk AND fkptjax is
    importable. Builds MgEmulatorCosmology + fkpt_pkemu_TracerPowerSpectrumMultipoles
    in the GR limit (mu=Sigma=1) and checks the multipoles are finite.
    """
    import os
    os.environ.setdefault("FOLPS_BACKEND", "jax")  # folps must run in jax mode

    from desilike.theories.galaxy_clustering import MgEmulatorCosmology

    if not os.path.isdir(MgEmulatorCosmology.EMU_PLIN_PATH):
        pytest.skip("trained MG emulators not available on this machine")
    if importlib.util.find_spec("fkptjax") is None:
        pytest.skip("fkptjax not installed")

    import numpy as np
    from desilike.theories.galaxy_clustering import fkpt_pkemu_TracerPowerSpectrumMultipoles

    z = 0.5
    cosmo = MgEmulatorCosmology(zs=[z])
    k = np.linspace(0.02, 0.20, 19)
    theory = fkpt_pkemu_TracerPowerSpectrumMultipoles()
    theory.init.update(prior_basis="standard", tracer="LRG1",
                       cosmo=cosmo, z=z, k=k, ells=(0, 2, 4), beyond_eds=True)
    theory()  # default (GR-limit) bias + cosmo params

    power = np.asarray(theory.power)
    assert power.shape == (3, len(k))
